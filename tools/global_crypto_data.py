...
class GetCoingeckoGlobalCryptoData:
    ...
    def get_global_crypto_data(self):
        ...
        return pd.DataFrame(
            {
                "timestamp": [pd.to_datetime(gcd["updated_at"], unit="s")],
    ...
    def push_to_db(self):
        ...
        try:
            self.get_global_crypto_data().to_sql(
                "global_crypto_data",
                ConnectionClient().postgres(),
                if_exists="append",
                index=False,
                schema="public",
            )
            ...
            logger.info(
                f"{datetime.now()} Data with that timestamp is already present in postgres"
            )
